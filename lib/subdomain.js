...
module.exports = function(options) {
  ...
  if (!options.base) {
    ...
    options.ignoreByRegex = options.ignoreByRegex || false;
  ...
  return function(request, response, next) {
    ...
    if (host === options.base
        ...
        || (options.ignoreByRegex instanceof RegExp && options.ignoreByRegex.test(request.originalUrl))
    ) {
      // not a subdomain or ignoring www subdomain or matches custom regex
      next();
    } else {
      // test for subdomain
